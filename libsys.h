...
struct LibsysInfo
{						   // schema //catalog
	char booksys_name[30]; // Book	//NEW
	FILE *booksys_data_fp;
	int book_repo_status;

	char studsys_name[30]; // Student	//NEW
	FILE *studsys_data_fp;
	int stud_repo_status;

	char issuesys_name[30]; // Issue	//NEW
	FILE *issuesys_data_fp;
	int issue_repo_status;

	FILE *book_libsys_ndx_fp; // BookIndexFile
	struct LIB_NdxInfo book_index_entries[LIB_MAX_INDEX_ENTRIES];
	int book_index_count;

	FILE* student_libsys_ndx_fp;
	struct Stud_NdxInfo student_index_entries[LIB_MAX_INDEX_ENTRIES];
	int student_index_count;
};

struct Book // Entity Structure
{
	int isbn;
	char title[40];
	char author[40];
...
struct Student // NEW //Entity Structure
{
	int rollno;
	char name[40];
	char address[40];
...
struct Issue // NEW //Link Structure(Helps to link differernt entities) //linking the student with book issued
{
	int rollno;
	int isbn;
};
